...
Como podemos observar los shows tenían una asistencia media de 10473, valor que oscilaba principalmente entre 8326 y 12621 personas. Además, podemos observar que no hay presencia de datos extremas debido a que la diferencia entre la media y la mediana es pequeña.
...
En este caso podemos encontrar un rango intercuartil de 4.04 lo que nos lleva a concluir que no hay una gran dispersión en el conjunto de datos.
...
De acuerdo a las gráficas podemos observar que la frecuencia mas alta se encuentra en el rango de las 11000 personas y la menor en el rango de las 6000.
...
Con respecto a la cantidad de semanas activas podemos observar una media de 1340 y una mediana de 1442, por lo que a primera vista no existe una presencia de valores atípicos.
...
Para las semanas de actividad obtenemos un rango intercuartil de 400, podemos concluir que la 50% intermedio esta comprendido entre 1100 y 1500 lo que indica cierto nivel de dispersión.
...
De acuerdo a las gráficas podemos observar la poca frecuencia de elementos alrededor de la mediana, y que la mayoría de los datos se concentran a partir del tercer cuartil.
...
```{r}
p1 <- pnorm(c(0,300,600,900),gross_grouped_mean,gross_grouped_sd)
p2 <- pnorm(c(300,600,900,1500),gross_grouped_mean,gross_grouped_sd)
class_probs <- p2-p1
class_probs
```
...
chisq_alpha <- qchisq(0.95,1,lower.tail=F)

est_chisq
chisq_alpha
```

Para el nivel de significancia escogido, el estadístico chi-cuadrado indica que de hecho el recaudo en millones de dólares no cuenta con distribución normal. 
